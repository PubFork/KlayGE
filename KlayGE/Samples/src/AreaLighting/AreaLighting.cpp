...
namespace
{
	...
	{
	public:
		PointLightSourceUpdate(LightSource& light, float move_speed)
			: light_(light), pos_(light.Position()), move_speed_(move_speed)
		{
		}
		...
		void operator()(float app_time, float elapsed_time)
		{
			...
			light_.ModelMatrix(MathLib::translation(sin(app_time * 1000 * move_speed_) + pos_.x(), pos_.y(), pos_.z()));
		}
...
void AreaLightingApp::OnCreate()
{
	...
	point_light_->BindUpdateFunc(PointLightSourceUpdate(*point_light_, 1 / 1000.0f));
	...
	sphere_area_light_->BindUpdateFunc(PointLightSourceUpdate(*sphere_area_light_, 1 / 1000.0f));
	...
	tube_area_light_->BindUpdateFunc(PointLightSourceUpdate(*tube_area_light_, 1 / 1000.0f));
