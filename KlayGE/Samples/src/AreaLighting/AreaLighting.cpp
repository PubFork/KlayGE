...
namespace
{
	...
	{
	public:
		LightNodeUpdate(SceneNode& node, float move_speed)
			: node_(node), pos_(MathLib::transform_coord(float3(0, 0, 0), node_.TransformToParent())), move_speed_(move_speed)
		{
		}
		...
		void operator()(float app_time, float elapsed_time)
		{
			...
			node_.TransformToParent(MathLib::translation(sin(app_time * 1000 * move_speed_) + pos_.x(), pos_.y(), pos_.z()));
		}
...
void AreaLightingApp::OnCreate()
{
	...
	light_node->TransformToParent(MathLib::translation(0.0f, 10.0f, 0.0f));
	light_node->OnMainThreadUpdate().Connect(LightNodeUpdate(*light_node, 1 / 1000.0f));
