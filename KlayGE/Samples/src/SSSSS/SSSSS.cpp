...
void SSSSSApp::OnCreate()
{
	...
	auto* light_node_ptr = light_node.get();
	light_node->OnMainThreadUpdate().Connect([this, light_node_ptr](float app_time, float elapse_time) {
		KFL_UNUSED(app_time);
		KFL_UNUSED(elapse_time);

		float3 const light_pos = float3(0, 5, 0) - light_camera_->ForwardVec() * 2.0f;
		light_node_ptr->TransformToParent(MathLib::inverse(MathLib::look_at_lh(light_pos, light_pos + light_camera_->ForwardVec())));
	});
