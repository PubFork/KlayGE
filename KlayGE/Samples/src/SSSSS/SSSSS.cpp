...
void SSSSSApp::OnCreate()
{
	...
	auto& light_ref = *light;
	light->BindUpdateFunc([this, &light_ref](float app_time, float elapse_time) {
		KFL_UNUSED(app_time);
		KFL_UNUSED(elapse_time);

		light_ref.Position(float3(0, 5, 0) - light_camera_->ForwardVec() * 2.0f);
		light_ref.Direction(light_camera_->ForwardVec());
	});
