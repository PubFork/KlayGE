...
void DeepGBuffersApp::OnCreate()
{
	...
	for (uint32_t i = 0; i < 2; ++i)
	{
		...
		auto* spot_light_node_ptr = spot_light_node.get();
		spot_light_node->OnMainThreadUpdate().Connect([spot_light_node_ptr, light_pos](float app_time, float elapsed_time)
		{
			...
			spot_light_node_ptr->TransformToParent(MathLib::inverse(MathLib::look_at_lh(new_pos, float3(0, 0, 0))));
		});
